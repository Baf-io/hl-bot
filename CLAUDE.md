...
- **⚠️ RISK-POLICY-CONTRACT CLAMP ACTIVE (2026-05-25, commit `93207a8`):** hard limits override everything below — **≤3x, MAX 1 open position, kill-switch −3%/day & −5%/week, NO averaging (adds rejected), PROBE-only (≤$50 notional/≤$5 risk) until a source is KEEP-validated, 40x tracker OFF, funding/cascade OFF.** Book FLATTENED to comply. The aggressive active-swing/scalp/add-mirror/tracker machinery below is SUSPENDED by the clamp (constants still present, gated off). **B progress — ALL LIVE:** ✅B1 native per-order stops (`_place_protective_stop` at the brain's explicit `stop_px`, else `COPY_SL_PCT`; force-closes if stop unconfirmed — fails safe; brain entries get ONLY this stop + TTL, NO legacy SL/TP). ✅B2 stop-derived sizing — LIVE entries sized by US: `notional = RISK_PER_TRADE_PCT(1%)×equity / stop_dist`, cap 3x (brain's `size_notional` IGNORED for live; probe keeps ≤$50). ✅B3 KEEP/WATCH/PROBE tiering — `KEEP_SOURCES` (.env) unlocks `mode=live`; non-KEEP → probe-only. **EmberCN = KEEP (2026-05-25).** ✅B4 two-box transport: **LXC-Claude X-scraper brain (node `bafscrape-1`/100.93.122.60) → HMAC-SHA256-signed POST → `src/execution/intake.py` on me (100.115.113.91:8787) → I re-verify sig + idempotency + EVERY cardinal independently.** `INTAKE_ACK_ONLY=false`=live. Lifecycle (entry+stop+TTL) owned by executor/guardian. **Leaderboard copy → SHADOW (paper, `COPY_SHADOW=true`)** — validates the strategy on paper, never touches the live book. Sources = X-scraped calls (EmberCN, OnchainLens) + copy-traders.
