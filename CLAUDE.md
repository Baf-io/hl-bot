...
9. **NEVER ADOPT A HELD POSITION (hard rule, user 2026-05-27):** a copier may ONLY open on a WITNESSED flat→position transition (fresh entry, new token) while we're flat on that coin. NEVER open into a position the source already held when we started watching — baseline-seed & lock it out until they go fully flat. No stale adoption, ever. Adopt-style/proportional-peg mirrors (`watch_scale_mirror.py`, 78aa-peg pattern) VIOLATE this and must be fresh-entry-gated. (Within a fresh position, following his adds/trims/exit is fine.) See [[never-adopt-held-positions]].
