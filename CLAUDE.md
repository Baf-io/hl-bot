...
- **⚠️ RISK-POLICY-CONTRACT CLAMP ACTIVE (2026-05-25, commit `93207a8`):** hard limits override everything below — **≤3x, MAX 1 open position, kill-switch −3%/day & −5%/week, NO averaging (adds rejected), PROBE-only (≤$50 notional/≤$5 risk) until a source is KEEP-validated, 40x tracker OFF, funding/cascade OFF.** Book FLATTENED to comply. The aggressive active-swing/scalp/add-mirror/tracker machinery below is SUSPENDED by the clamp (constants still present, gated off). **B progress:** ✅B1 native per-order stops live (`_place_protective_stop` at `COPY_SL_PCT`; force-closes if stop unconfirmed — fails safe). **Pending:** B2 stop-derived 1%-equity-risk sizing, B3 KEEP/WATCH/PROBE source tiering, B4 two-box transport = **LXC-Claude (X-scraper signal box) → HMAC-signed → me (HL-bot executor) independently re-checks every rule**. Sources = X-scraped calls + copy-traders.
