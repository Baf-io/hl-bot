...
- **Sizing:** equal-weight to `COPY_TARGET_DEPLOY=0.85`, capped `MAX_POSITION_SIZE_PCT=0.15` margin/pos; gate `COPY_MIN_CONVICTION_PCT=0.05` (their margin ≥5% of their acct to copy/vote); `MIN_POSITION_NOTIONAL=50`; `COPY_MAX_COPY_LEVERAGE=10`. **Vol-scaled leverage (`VOL_SCALED_LEV=true`):** per-coin lev capped to `STOP_LOSS_MARGIN_PCT/(STOP_NOISE_ATR=0.5·dailyATR)` so the −20% stop always lands ≥0.5 ATR away (HYPE→~5x, SOL→7x, BTC/ETH→10x). Applies to NEW entries only.
- **Exits (HARD STOP + BANK-AND-RIDE, `RIDE_WINNERS_ENABLED`, `_ride_winners`):** HARD stop −`STOP_LOSS_MARGIN_PCT=0.20` of margin (price = pct/lev), **NO ATR floor** (old `max()` floor made it −47/−62% & fired 0×), enforced even if ATR missing. BANK `BANK_FRACTION=0.50` at +`BANK_AT_MARGIN_RET=0.40` margin (=+2R), then runner trails: after peak clears `RIDE_ACTIVATE_ATR=1.0`×ATR, exit on `RIDE_GIVEBACK_ATR=1.0`×ATR retrace. Guardian −70% nuclear = deep backstop. See `docs/ENTRY_EXIT_PLAN.md`.
