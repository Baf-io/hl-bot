...
- **Add-mirroring (`ADD_MIRROR_ENABLED=true`, scale-IN only):** when the source trader grows a held coin's notional ≥`ADD_MIN_FRAC=0.10` poll-over-poll AND the coin's daily trend (`close vs SMA(TREND_SMA_DAYS=5)`) aligns with our direction, we ADD `min(their %, ADD_STEP_MAX=0.50)`×our-margin (capped at the per-pos margin cap) and Telegram/ntfy-notify. **TRIMS are NOT mirrored** (their trims are early/negative-edge — data: their adds 90–100% WR in trends, trims 0–35%); exits stay ours. New `action="add"` path: `risk.approve` bypasses one-per-coin, `risk.add_to_position` does the weighted-avg entry, executor `_add_to_position` grows it. Verified 5/5 dry-run.
